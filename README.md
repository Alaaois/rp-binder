...
Важно: проект является helper-tool и не содержит автоматизацию игры, инжект, чтение памяти или обходы античита.
...
- Биндер с режимами `copy`, `paste`, `paste_enter`.
...
- `hotkey` (string)
- `enabled` (bool)
- `send_mode` (`copy` | `paste` | `paste_enter`)
- `delay_ms` (0..5000)
...
3. Не добавляйте функционал, выходящий за границы helper-tool.
